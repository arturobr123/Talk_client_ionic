...
export class MessagesPage {
  ...
  ionViewDidLoad() {
    ...
    console.log(firebase.auth().currentUser.uid);
     window.localStorage['uid_Firebase'] =firebase.auth().currentUser.uid;
    this.createUserData();
    this.searchFriend = '';
      ...
      .then((account) => {
        ...
        if (!account.val()) {
          ...
          if (user.displayName || providerData.displayName) {
            name = window.localStorage['current_username']; //user.displayName
            name = window.localStorage['current_username']; //providerData.displayName
          } else {
            name = window.localStorage['current_username']; // "Firebase User"
          }
          ...
          if (user.photoURL || providerData.photoURL) {
            img = window.localStorage['current_user_avatar'];
            img = window.localStorage['current_user_avatar'];
          } else {
            img = window.localStorage['current_user_avatar'];
          }
